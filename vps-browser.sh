...
if [ "$(docker ps -q -f name=browser)" ]; then
    ...
    docker run -d --name browser -e TITLE=ZunXBT -e DISPLAY=:1 -e PUID=1000 -e PGID=1000 -e CUSTOM_USER="$USERNAME" -e PASSWORD="$PASSWORD" -e LANGUAGE=en_US.UTF-8 -v "$HOME/chromium/config:/config" -p 3000:3000 -p 3001:3001 --shm-size="1gb" --restart unless-stopped lscr.io/linuxserver/chromium:latest
    if [ $? -eq 0 ]; then
